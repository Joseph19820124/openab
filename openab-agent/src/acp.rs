...
impl AcpServer {
    ...
    fn handle_session_new(&mut self, id: u64) -> String {
        ...
        let provider: Box<dyn crate::llm::LlmProvider> = match provider_choice.as_str() {
            ...
            "gemini" | "google" => match crate::llm::GeminiProvider::from_env() {
                Ok(p) => Box::new(p),
                Err(e) => return self.error_response(id, -32000, &e),
            },
            ...
            _ => {
                // Auto-detect: try API keys first (Anthropic, then Gemini), then OAuth token
                match AnthropicProvider::from_env() {
                    Ok(p) => Box::new(p),
                    Err(_) => match crate::llm::GeminiProvider::from_env() {
                        Ok(p) => Box::new(p),
